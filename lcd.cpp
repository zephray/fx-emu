            ...
            case REG_LCDDAT:
				...
				if (lcd_reg[REG_POSTID] & 0x04) { //LCDPE ON
					if (lcd_reg[REG_POSTID] & 0x40) { //Auto Increase
						if (lcd_reg[REG_LCDARL] < 0x61) lcd_reg[REG_LCDARL]++; else { lcd_reg[REG_LCDARL] = 0;  }
					}
					else {
						if (lcd_reg[REG_LCDARL] > 0x00) lcd_reg[REG_LCDARL]--; else { lcd_reg[REG_LCDARL] = 0x61; }
					}
				}
				break;
            default:
...
void lcd_write_byte(uint8_t addr, uint8_t byte) {
    if (addr < 0x30) {
        ...
        switch (addr) {
            case REG_LCDDAT:
            lcd_fb[(((uint16_t)(lcd_reg[REG_LCDARH] & 0x03)) << 7) | (lcd_reg[REG_LCDARL])] = byte;
			if (lcd_reg[REG_POSTID] & 0x04) { //LCDPE ON
				if (lcd_reg[REG_POSTID] & 0x40) { //Auto Increase
					if (lcd_reg[REG_LCDARL] < 0x61) lcd_reg[REG_LCDARL]++; else { lcd_reg[REG_LCDARL] = 0; }
				}
